namespace SuitSupply.Core.Azure
{
    public class AzureConstants
    {
        public const string TokenIssuer = "RootManageSharedAccessKey";
        public const string TokenAccessKey = "aaB1w2B6y/siCW9wHyeyGQO1u+KNe5aIe8QftpJukq8=";
        public const string ServiceUriScheme = "sb";
        public const string ServiceNamespace = "AppverseBus";
        public const string ServicePath = "";
        public const string SuitTopic = "SuitCommand";

        public class AzureSubscribtions
        {
            public const string AddProduct= "AddProductSubscriber";
            public const string AddNumber = "AddNumberSubscriber";

        }

    }
}